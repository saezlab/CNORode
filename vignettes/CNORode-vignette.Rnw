...
algorithm is part of the MEIGOR toolbox which is available on BioConductor and it can be downloaded from
\url{https://www.bioconductor.org/packages/release/bioc/html/MEIGOR.html}. Once downloaded, install
MEIGOR by typing:
...
R=crossvalidateODE(CNOlist = cnolist, model = model, type="datapoint", nfolds=3, parallel = TRUE, ode_parameters = ode_parameters, paramsSSm = paramsSSm)
@
