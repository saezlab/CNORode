...
# we apply a small trick here with the initialValueMatrix: we set if all the states
# are measured as signals and we set the valueSignals according to the initialValueMatrix. 
# this way we dont have to touch the C-code. 
...
(
	...
	maxErrTestsFails1=50,   initialValueMatrix1=NULL
)
{
	if(is.null(initialValueMatrix1)){
		initialValueMatrix1 =  createLBodeInitialConditions(model = model1,data = cnolist1)
	}
	...
	(
		...
		maxErrTestsFails=maxErrTestsFails1, initialValueMatrix=initialValueMatrix1
	)
	{	
		...
		# change valueSignals to initialValueMatrix because 
		valueSignals <- as.double(t(initialValueMatrix))
		...
		#indices
		nOutputSignals <- as.integer(length(indices$signals))
		nSignals <- as.integer(ncol(initialValueMatrix))
		
		indexOutputSignals <- as.integer(as.vector(indices$signals))
		indexSignals <- as.integer(as.vector(1:nSignals))
		...
		res = .Call("sim_logic_ode",interMat,notMat,adjMatrix,nRows,nCols,nPars,timeSignals,
					...
					break_at_1st_fail,PACKAGE = "CNORode");
