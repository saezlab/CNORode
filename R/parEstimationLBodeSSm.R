...
(
		...
		maxNumSteps=100000,		maxErrTestsFails=50,	nan_fac=1, lambda=0, boot_seed=sample(1:10000,1)
)
{
   ...
   tryCatch({library(MEIGOR)}, error=function(e){print("MEIGOR (essR) package not found.
	...
	time,verbose,transfer_function,reltol,atol,maxStepSize,maxNumSteps,maxErrTestsFails, lambda=lambda, boot_seed=boot_seed);
